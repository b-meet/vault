...
import { checkAuthState, loginWithEmail, loginWithGoogle, logout, refreshUserToken, signupWithEmail } from '../thunk/userThunk';
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(checkAuthState.fulfilled, (state, action: PayloadAction<{ user: UserState['user']; token: string; refreshToken: string } | null>) => {
        state.loading = false;
        if (action.payload) {
          state.user = action.payload.user;
          state.token = action.payload.token;
